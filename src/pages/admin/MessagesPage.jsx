...
/**
 * MessagesPage - Admin page for managing user messages and details.
 *
 * Features:
 * - List, search, and view users and their messages.
 * - View user details, stats, and perform admin actions (activate, reset password, delete).
...
export default function MessagesPage() {
  const { theme } = useThemeStore();
    ...
    const memoFilteredUsers = useMemo(() => filteredUsers, [filteredUsers]);
    ...
    }, [id, users, setSelectedUser, navigate]);
    ...
    if (isUsersLoading) {
      return (
        <div className="flex items-center justify-center h-32">
          <span className="loading loading-spinner loading-lg text-primary" aria-label="Loading users"></span>
        </div>
      );
    }
    ...
    if (!users || users.length === 0) {
      return (
        <div className="text-base-content/70 px-2 pt-4">No users found.</div>
      );
    }
    ...
    if (id && selectedUser) {
      ...
      return (
        <div className={`${textColor} h-[calc(100vh-4rem)] pt-10 w-full flex justify-center items-center`}>
          ...
          <Link to="/admin/messages" className="btn btn-primary fixed bottom-6 right-6 z-40 shadow-lg rounded-full md:hidden" title="New Message" aria-label="New Message">
      ...
      <div data-theme={theme} className="px-2 pt-4">
        ...
        <Card>
          <CardHeader className="pb-2 flex flex-row items-center justify-between">
            ...
            <span className="text-base-content/60 text-sm">({memoFilteredUsers.length})</span>
          </CardHeader>
          <CardContent className="pt-2">
            {memoFilteredUsers.length === 0 ? (
              <div className="text-base-content/70">No users found.</div>
            ) : (
              <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
                {memoFilteredUsers.map((user) => {
                  ...
                  return (
                    <div
                      ...
                      tabIndex={0}
                      aria-label={`User card for ${user.name || user.email}`}
                    >
                      ...
                      <div className="flex gap-2 mt-2">
                        <Link to={`/admin/messages/${user._id}`} className="btn btn-sm btn-primary flex items-center gap-1 rounded-full px-4 py-2 text-base-content font-medium shadow hover:shadow-md" title="Open chat" aria-label={`Open chat with ${user.name || user.email}`}>
        ...
        {detailsModalOpen && detailsUser && (
          <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center" role="dialog" aria-modal="true" tabIndex={-1} onKeyDown={e => e.key === 'Escape' && setDetailsModalOpen(false)}>
            <div className="bg-base-100 text-base-content p-6 rounded-lg w-full max-w-md shadow-lg relative">
              ...
              <div className="flex flex-col gap-2 mt-4">
                <button
                  ...
                  aria-label={detailsUser.isActive ? 'Deactivate user' : 'Activate user'}
                >
