...
const ChatContainer = ({ showHeader = true }) => {
  ...
  return (
    <div className={`flex-1 flex flex-col min-h-0 rounded-lg shadow-md ${theme === 'dark' ? 'bg-base-200' : 'bg-base-100'}`}>
      {showHeader && (
        <div className="sticky top-0 z-20 bg-inherit border-b border-base-300 shadow-md">
          <ChatHeader />
        </div>
      )}
      ...
      <div className={`sticky bottom-0 z-10  p-2 ${theme === 'dark' ? 'border-base-300 bg-base-200' : 'border-base-100'}`}>
        <MessageInput
          onSend={({ text, image }) => {
